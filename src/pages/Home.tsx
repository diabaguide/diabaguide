...
import { fetchAnnonces, type Annonce } from '../lib/annonces';
...
/* Annonces de services rédigées par l'équipe (console : /equipe/annonces).
   La base ne rend que les annonces actives et dans leur période : la section
   disparaît d'elle-même quand il n'y a rien à montrer, et l'échec d'un
   chargement (module pas encore installé) ne casse pas l'accueil. */
function AnnoncesDiaba() {
  const { tr } = useI18n();
  const [annonces, setAnnonces] = useState<Annonce[]>([]);
  useEffect(() => {
    let alive = true;
    fetchAnnonces().then((liste) => { if (alive) setAnnonces(liste); });
    return () => { alive = false; };
  }, []);
